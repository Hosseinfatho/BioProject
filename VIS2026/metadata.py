...
def get_channel_min_max(
    dataset_url: str = DATASET_S3,
    metadata_url: Optional[str] = METADATA_HTTP_URL,
    fs: Optional[s3fs.S3FileSystem] = None,
) -> List[Dict[str, Any]]:
    """
    Return for each channel: name, min, max (intensity range).
    Channel names from OME; min/max from Zarr arrays (by index: channel i = Zarr key str(i)).
    """
    channel_names = []
    try:
        channel_names = get_all_channel_names(metadata_url=metadata_url)
    except Exception:
        pass
    if not channel_names:
        # Fallback: use Zarr keys as names
        ranges = get_intensity_range_from_zarr(dataset_url, fs)
        return [
            {"name": str(r["channel_key"]), "min": r.get("min"), "max": r.get("max")}
            for r in ranges
        ]
    ranges = get_intensity_range_from_zarr(dataset_url, fs)
    out = []
    for i, name in enumerate(channel_names):
        rec = {"name": name, "min": None, "max": None}
        # Match by index: channel i <-> Zarr key str(i)
        for r in ranges:
            if str(r.get("channel_key")) == str(i):
                rec["min"] = r.get("min")
                rec["max"] = r.get("max")
                break
        out.append(rec)
...
def read_metadata(
    ...
    # Add min/max per channel from Zarr (by index: channel i = Zarr array key str(i))
    try:
        ranges = get_intensity_range_from_zarr(dataset_url, fs)
...
if __name__ == "__main__":
    ...
    with open(out_dir / "channel_min_max.json", "w") as f:
        json.dump(channel_min_max, f, indent=2, default=str)
    print(f"\nSaved: {out_dir / 'metadata.json'}, {out_dir / 'channel_min_max.json'}")
